...
void editfile(int d, char *filename, char *basename)
{
    ...
    editmodflg = 0;
    if (viewrec->broken) {
        if (0 != getchoice(1, " Edit ", "File contains too long lines.", 0, " Break lines ",
                           " Cancel editing ", 0))
            return;
        RecBreak(viewrec);
        editmodflg = 1;
    ...
    VMPutString(LINES - 1, 0,
                "\0011\16Help  \17 2\16Save  \17 3\16Raw   \17 4\16      \17 5\16Top   \17 "
                "6\16Bottom\17 7\16Search\17 8\16Home  \17 9\16Spaces\01710\16Quit \17\2");
    for (;;) {
        ...
        for (;;) {
            ...
            case meta('F'): /* f6 - bottom */
                if (soff == lastpos(sline) && sline >= viewrec->len - 1)
                    continue;
            ...
            case meta('J'): /* f0 - quit */
                if (editmodflg)
                    switch (getchoice(1, " Edit ", "You've made changes since the last save.", 0,
